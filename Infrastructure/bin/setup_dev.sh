...
oc set probe dc/parksmap-dev --readiness --get-url=http://:8080/ws/healthz/ --initial-delay-seconds=30
oc set probe dc/parksmap-dev --liveness --get-url=http://:8080/ws/healthz/ --initial-delay-seconds=30
...
oc rollout pause dc national-parks-dev
#Add Liveliness and readiness probes
...
oc patch dc/national-parks-dev --patch '{"spec":{"strategy":{"rollingParams":{"post":{"failurePolicy": "retry","execNewPod":{"containerName":"national-parks-dev","command":["/bin/sh","-c","curl http://${SERVICE_NAME}:8080/ws/data/load/"], "env": [{"name": "SERVICE_NAME", "value":"national-parks-dev"}]}}}}}}'
oc rollout resume dc national-parks-dev
...
oc rollout pause dc mlb-parks-dev
#Add Liveliness and readiness probes
...
#Add post deployment hook!
oc patch dc/mlb-parks-dev --patch '{"spec":{"strategy":{"rollingParams":{"post":{"failurePolicy": "retry","execNewPod":{"containerName":"mlb-parks-dev","command":["/bin/sh","-c","curl http://${SERVICE_NAME}:8080/ws/data/load/"], "env": [{"name": "SERVICE_NAME", "value":"mlb-parks-dev"}]}}}}}}'
oc rollout resume dc mlb-parks-dev
...
oc rollout resume dc parksmap-dev